        ...
        if (column != 0) {
            ...
            for (ii = 0; ii < client_first.length; ii++) {
               ...
               // this little function right here reads all the calendars and returns the data we need to output
                var All_Cal_Info = scan_calendar(PML, client_first[ii], client_last[ii], begin_prev_month, end_prev_month, begin_this_month, end_this_month, begin_next_month, end_next_month);
                var bill_boolean = 0; 
                ...
                for (jj = 0; jj < All_Cal_Info.length; jj++) {
                    ...
                    tracker.getRange(first_empty + jj, 5, 1, 1).setValue((lesson_cost[ii] / 60) * All_Cal_Info[jj].lesson_length); // lesson cost
